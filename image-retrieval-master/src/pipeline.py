...
def find_clusters(descriptors, k=256):
    ...
    kmeans = MiniBatchKMeans(
        n_clusters=k, init="k-means++", verbose=False, random_state=123
    )
...
def main():
    ...
    with open("result_query.out", "wb") as outfile:
        for i, val_img in enumerate(val_images):
            ...
            for i in range(len(similar_images)):
                line += (
                    " "
                    + str(similar_images[i][0][0][0])
                    + " "
                    + str(similar_images[i][1])
                )
            line += "\n"
    ...
    subprocess.call(
        [
            "python3",
            "utils/compute_map.py",
            "converted_result_query.out",
            "validation_gt.dat",
        ]
    )
