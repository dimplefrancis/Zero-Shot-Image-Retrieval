...
class RandomProjection(object):
    ...
    def make_samples(self, db, verbose=False):
        if verbose:
            print(
                "Use features {}, {} RandomProject, keep {}".format(
...
def evaluate_feats(
    ...
    result = open(
        os.path.join(
            result_dir,
            "feature_reduction-{}-keep{}-{}-{}feats.csv".format(
                project_type, keep_rate, d_type, N
            ),
        ),
        "w",
    )
    ...
    for combination in combinations:
        fusion = RandomProjection(
            features=list(combination), keep_rate=keep_rate, project_type=project_type
        )
