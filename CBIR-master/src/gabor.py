...
if sigma and not bandwidth:
    assert len(gabor_kernels) == theta * len(frequency) * len(
        sigma
    ), "kernel nums error in make_gabor_kernel()"
    ...
    assert len(gabor_kernels) == theta * len(frequency) * (
        len(sigma) + len(bandwidth)
    ), "kernel nums error in make_gabor_kernel()"
elif not sigma and not bandwidth:
    assert len(gabor_kernels) == theta * len(
        frequency
    ), "kernel nums error in make_gabor_kernel()"
...
class Gabor(object):
    ...
    def _gabor(
        self,
        image,
        kernels=make_gabor_kernel(theta, frequency, sigma, bandwidth),
        normalize=True,
    ):
    ...
    def make_samples(self, db, verbose=True):
        if h_type == "global":
            ...
            sample_cache = (
                "gabor-{}-n_slice{}-theta{}-frequency{}-sigma{}-bandwidth{}".format(
